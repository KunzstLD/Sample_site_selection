...
Numbers indicate each delineated cluster. Values for each variable are z-standardised (Stand_value), i.e. they can be interpreted in terms of differences in standard deviations from the mean. Colored sites are those sites that have the overall smallest sum of squared differences to the centroid/mean. 
...
Sites ordered according to their clusters delineated by hierarchical cluster analysis (Wards method). 
Sites are also ordered according to their squared sum of differences to the centroid (Sum Difference). 

```{r}
cluster_result <- unique(test_dat_lf[order(Group), .(Site, Group, Sum_diff)])
reactable(
  cluster_result[order(Group, Sum_diff), .(Site,
                                           Group,
